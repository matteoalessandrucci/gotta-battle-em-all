...
class BattleSimulator:
    def __init__(self, agent_type=0, use_ux=True, n_battles=3, batch_folder=None):
        ...
        if self.agent_type == 1:
            self.agent0 = MonteCarloAgent() #MonteCarloAgent()
    ...
    def log_to_file(self, log_text, filename=None):
        ...
        # Usa il filename specificato o genera uno nuovo
        if not filename:
            timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            filename = f"{'test_batch_' if self.batch_folder else 'battle_'}log_{agent_type}_{timestamp}.txt"
        else:
            # Aggiungi l'agente e la data al nome del file
            timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            filename = f"{filename}_{agent_type}_{timestamp}.txt"
    ...
    def run_battles(self):
        ...
        for battle in range(self.n_battles):
            ...
            while not t:
                ...
                # Log and print simulations and exploration constant for MCTS
                if self.agent_type == 1:  # MCTS Agent
                    if isinstance(self.agent0, MonteCarloAgent):
                        print(f"Battle {battle + 1}, Turn {battle_turns + 1}: "
                            f"Simulations = {self.agent0.simulations}, "
                            f"Exploration Constant = {self.agent0.exploration_constant:.2f}")
...
if __name__ == "__main__":
    ...
    if test_batch:
        ...
        # Calculate total battles as batch_runs * n_battles
        total_battles = batch_runs * n_battles
        win_rate_team0 = total_wins_team0 / batch_runs * 100
        win_rate_team1 = total_wins_team1 / batch_runs * 100
